  ...
  <div class="pl-3">
    <nav aria-label="breadcrumb">
      <ol class="breadcrumb mb-0 p-0">
        ...
        <li class="breadcrumb-item active" aria-current="page">Subsubcategories</li>
      </ol>
    </nav>
  </div>
</div>
<div class="card border-lg-top-info radius-15">
  <div class="card-header border-bottom-0 mb-4">
    <div class="d-flex align-items-center">
      <div>
        <h5>Controll Subsubcategories</h5>
      </div>
      <div class="ml-auto">
        @if (Auth::user()->hasPermission('app.categories.create'))
        <a class="btn btn-primary px-3" href="{{ route('app.subsubcategories.create') }}" data-toggle="tooltip" title="Add new subSubcategory &#9989"><i class="bx bx-plus mr-1"></i>Add</a>
        @endif
      </div>
    </div>
  </div>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              ...
              <a class="btn btn-sm btn-success" href="{{ route('app.subsubcategories.edit', $subsubcategory->id) }}" data-toggle="tooltip" title="Edit &#128221"><i class="fadeIn animated bx bx-edit"></i>
              </a>
              @endif
              @if (Auth::user()->hasPermission('app.categories.destroy'))
              <form action="{{ route('app.subsubcategories.destroy', $subsubcategory->id) }}" style="display: inline-block" method="POST">
                @csrf
                @method('DELETE')
